...
export const AnimatedDots: React.FC<AnimatedDotsProps> = ({
  ...
  useEffect(() => {
    ...
    class Dot {
      ...
      constructor(i: number) {
        ...
        // Distribute dots perfectly across the viewport width
        this.x = (i / dotsNum) * width;
        this.y = -this.radius;
      }
    ...
    const draw = () => {
      // Use clearRect to ensure no streaks on transparent background
  ...
  return (
    <div
      className={`relative ${fullScreen ? "w-screen h-screen" : ""} ${className} pointer-events-none`}
    >
